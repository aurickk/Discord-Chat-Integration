...
public class DiscordChatIntegrationClient implements ClientModInitializer {
	...
	public void onInitializeClient() {
		...
		// Show status automatically when joining a world/server
	...
	private void startWebSocketServer(int port) {
		...
		new Thread(() -> {
			try {
				server.start();
				// Wait a moment to ensure server actually started
				Thread.sleep(200);
				if (server.isRunning()) {
				...
				if (isBindError) {
					...
					// Show warning to player with instructions
					Minecraft client = Minecraft.getInstance();
					if (client != null) {
						client.execute(() -> {
